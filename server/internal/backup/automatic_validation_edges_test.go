...
func TestRestoreAndDeleteValidateObjectKeysBeforeRemoteCall(t *testing.T) {
	...
	if _, err := service.Restore(context.Background(), "other/xlyra-backup-20260621-030000.zip.xlyra", ImportOptions{}); err == nil || !strings.Contains(err.Error(), "outside configured prefix") {
