...
func NewHandlerWithBackup(logger *slog.Logger, confFile *config.ConfigFile, db *store.Store, masterKey string, downloadService *downloads.Service, playgroundRoot string, preRestore func(context.Context) error, postRestore func(context.Context) error, databaseRestored func(context.Context) error, timeZones ...config.TimeZone) Handler {
	timeZone := config.TimeZoneOrDefault(timeZones...)
	handler := NewHandler(logger, confFile, db)
	handler.backups = backup.NewService(db, confFile, masterKey, playgroundRoot, timeZone).WithRestoreHooks(preRestore, postRestore).WithDatabaseRestored(databaseRestored)
