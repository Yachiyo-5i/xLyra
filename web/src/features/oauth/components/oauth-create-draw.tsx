...
export function OAuthCreateDraw({
  ...
  const dialogTitle = title ?? t('create.title')
  ...
  const confirmButton = state.authorize ? (
    <Button
  ...
  const cancelButton = state.authorize ? (
    <Button
      variant="ghost"
    ...
    return (
      <Draw open={open} onOpenChange={onOpenChange}>
        <DrawContent side="right" onOpenAutoFocus={(event) => event.preventDefault()}>
          <DrawHeader>
            <DrawTitle>{dialogTitle}</DrawTitle>
          </DrawHeader>
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent size="md" onOpenAutoFocus={(event) => event.preventDefault()}>
        ...
        <DialogBody className="min-h-0 min-w-0 max-w-full flex-1 space-y-5 overflow-y-auto">
          {body}
        </DialogBody>
        <DialogFooter cancel={cancelButton} confirm={confirmButton} />
      </DialogContent>
    </Dialog>
  )
}
